<!-- Navigation Bar Start -->
<nav>
    <!-- Left Nav Start -->
    <div class="nav_left">
        <svg class='facebook_logo' width="500" height="504" viewBox="0 0 500 504" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="250" cy="254" r="250" fill="url(#paint0_linear_3_16)"/>
            <path d="M335.011 220.364V272.068H167.341V220.364H335.011ZM209.259 504V193.588C209.259 174.507 213.198 158.626 221.077 145.946C229.079 133.266 239.789 123.787 253.207 117.509C266.626 111.23 281.522 108.091 297.895 108.091C309.467 108.091 319.746 109.014 328.733 110.861C337.72 112.707 344.367 114.369 348.676 115.847L335.381 167.551C332.549 166.689 328.979 165.828 324.67 164.966C320.362 163.981 315.561 163.489 310.267 163.489C297.833 163.489 289.031 166.505 283.861 172.537C278.813 178.446 276.29 186.94 276.29 198.02V504H209.259Z" fill="white"/>
            <defs>
            <linearGradient id="paint0_linear_3_16" x1="260" y1="504" x2="250" y2="-10" gradientUnits="userSpaceOnUse">
            <stop stop-color="#0073FF"/>
            <stop offset="1" stop-color="#80B9FF"/>
            </linearGradient>
            </defs>
        </svg>
        <div class="search_bar">
            <svg class='search_icon' width="409" height="386" viewBox="0 0 409 386" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="165" cy="165" r="150" stroke="#727272" stroke-width="30"/>
                <rect x="271" y="272.339" width="34.4203" height="160.034" rx="7" transform="rotate(-45 271 272.339)" fill="#727272"/>
            </svg>
            <input type="text" class="search" placeholder='Search Facekbook'/>
        </div>
        <div class="menu_bar_container">
            <div id="menubar" class="menu_bar" style="margin-left:10px;display: flex; align-items:center; justify-content:center;position:absolute;top:5px;left:0; border-radius:50%; background-color: white;">
                <a aria-label="More" class="oajrlxb2 g5ia77u1 qu0x051f esr5mh6w e9989ue4 r7d6kgcz rq0escxv nhd2j8a9 p7hjln8o kvgmc6g5 cxmmr5t8 oygrvhab hcukyx3x jb3vyjys rz4wbd8a qt6c0cv9 a8nywdso i1ao9s8h esuyzwwr f1sip0of n00je7tq arfg74bv qs9ysxi8 k77z8yql abiwlrkh p8dawk7l lzcic4wl bp9cbjyn j83agx80 cbu4d94t datstx6m taijpn5t l9j0dhe7 k4urcfbm" onclick="" role="link" tabindex="0"><span class="l9j0dhe7"><svg viewBox="0 0 28 28" class="a8c37x1j ms05siws l3qrxjdp b7h9ocf4 py1f6qlh" fill="#727272" height="28" width="28"><path d="M23.5 4a1.5 1.5 0 110 3h-19a1.5 1.5 0 110-3h19zm0 18a1.5 1.5 0 110 3h-19a1.5 1.5 0 110-3h19zm0-9a1.5 1.5 0 110 3h-19a1.5 1.5 0 110-3h19z"></path></svg><!--$--><span class="pmk7jnqg h5g66v2i nezaghv5"></span><!--/$--></span><div class="n00je7tq arfg74bv qs9ysxi8 k77z8yql i09qtzwb n7fi1qx3 b5wmifdl hzruof5a pmk7jnqg j9ispegn kr520xx4 c5ndavph art1omkt ot9fgl3s" data-visualcompletion="ignore" style="border-radius:8px;bottom:4px;left:0;right:0;top:4px"></div></a>
            </div>
            <div id="closemenubar" class="close_menu_bar" style="margin-left:8px;background-color:transparent; display: flex; align-items:center; justify-content:center;">
                <svg width="32" height="32" viewBox="0 0 185 185" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle r="92.5" transform="matrix(-1 0 0 1 92.5 92.5)" fill="white"/>
                <line x1="146.701" y1="149.308" x2="37.6967" y2="40.3033" stroke="#606266" stroke-width="15"/>
                <line x1="38.1944" y1="147.701" x2="147.199" y2="38.6967" stroke="#606266" stroke-width="15"/>
                </svg>            
            </div>
        </div>
        </div>
    </div>
    <!-- Left Nav End -->

    <!-- Mid Nav Start -->
    <div class="nav_mid">
        <ul class="navlinks">
            <a href="{{route('authcheck')}}">
                <li class="icon_link">
                    <svg viewBox="0 0 25 25" class="a8c37x1j ms05siws l3qrxjdp b7h9ocf4 g28tu32o" fill="#1b74e4" height="23" width="23"><path d="M25.825 12.29C25.824 12.289 25.823 12.288 25.821 12.286L15.027 2.937C14.752 2.675 14.392 2.527 13.989 2.521 13.608 2.527 13.248 2.675 13.001 2.912L2.175 12.29C1.756 12.658 1.629 13.245 1.868 13.759 2.079 14.215 2.567 14.479 3.069 14.479L5 14.479 5 23.729C5 24.695 5.784 25.479 6.75 25.479L11 25.479C11.552 25.479 12 25.031 12 24.479L12 18.309C12 18.126 12.148 17.979 12.33 17.979L15.67 17.979C15.852 17.979 16 18.126 16 18.309L16 24.479C16 25.031 16.448 25.479 17 25.479L21.25 25.479C22.217 25.479 23 24.695 23 23.729L23 14.479 24.931 14.479C25.433 14.479 25.921 14.215 26.132 13.759 26.371 13.245 26.244 12.658 25.825 12.29"></path></svg>
                </li>
            </a>
            <li class="icon_link">
                <a href="watch">
                    <svg viewBox="0 0 28 28" class="a8c37x1j ms05siws l3qrxjdp b7h9ocf4 py1f6qlh" fill="#606266" height="28" width="28"><path d="M8.75 25.25C8.336 25.25 8 24.914 8 24.5 8 24.086 8.336 23.75 8.75 23.75L19.25 23.75C19.664 23.75 20 24.086 20 24.5 20 24.914 19.664 25.25 19.25 25.25L8.75 25.25ZM17.163 12.846 12.055 15.923C11.591 16.202 11 15.869 11 15.327L11 9.172C11 8.631 11.591 8.297 12.055 8.576L17.163 11.654C17.612 11.924 17.612 12.575 17.163 12.846ZM21.75 20.25C22.992 20.25 24 19.242 24 18L24 6.5C24 5.258 22.992 4.25 21.75 4.25L6.25 4.25C5.008 4.25 4 5.258 4 6.5L4 18C4 19.242 5.008 20.25 6.25 20.25L21.75 20.25ZM21.75 21.75 6.25 21.75C4.179 21.75 2.5 20.071 2.5 18L2.5 6.5C2.5 4.429 4.179 2.75 6.25 2.75L21.75 2.75C23.821 2.75 25.5 4.429 25.5 6.5L25.5 18C25.5 20.071 23.821 21.75 21.75 21.75Z"></path></svg>
                </a>
            </li>
            <li class="icon_link">
                <a href="marketplace">
                    <svg viewBox="0 0 28 28" class="a8c37x1j ms05siws l3qrxjdp b7h9ocf4 py1f6qlh" fill="#606266" height="28" width="28"><path d="M17.5 23.75 21.75 23.75C22.164 23.75 22.5 23.414 22.5 23L22.5 14 22.531 14C22.364 13.917 22.206 13.815 22.061 13.694L21.66 13.359C21.567 13.283 21.433 13.283 21.34 13.36L21.176 13.497C20.591 13.983 19.855 14.25 19.095 14.25L18.869 14.25C18.114 14.25 17.382 13.987 16.8 13.506L16.616 13.354C16.523 13.278 16.39 13.278 16.298 13.354L16.113 13.507C15.53 13.987 14.798 14.25 14.044 14.25L13.907 14.25C13.162 14.25 12.439 13.994 11.861 13.525L11.645 13.35C11.552 13.275 11.419 13.276 11.328 13.352L11.155 13.497C10.57 13.984 9.834 14.25 9.074 14.25L8.896 14.25C8.143 14.25 7.414 13.989 6.832 13.511L6.638 13.351C6.545 13.275 6.413 13.275 6.32 13.351L5.849 13.739C5.726 13.84 5.592 13.928 5.452 14L5.5 14 5.5 23C5.5 23.414 5.836 23.75 6.25 23.75L10.5 23.75 10.5 17.5C10.5 16.81 11.06 16.25 11.75 16.25L16.25 16.25C16.94 16.25 17.5 16.81 17.5 17.5L17.5 23.75ZM3.673 8.75 24.327 8.75C24.3 8.66 24.271 8.571 24.238 8.483L23.087 5.355C22.823 4.688 22.178 4.25 21.461 4.25L6.54 4.25C5.822 4.25 5.177 4.688 4.919 5.338L3.762 8.483C3.729 8.571 3.7 8.66 3.673 8.75ZM24.5 10.25 3.5 10.25 3.5 12C3.5 12.414 3.836 12.75 4.25 12.75L4.421 12.75C4.595 12.75 4.763 12.69 4.897 12.58L5.368 12.193C6.013 11.662 6.945 11.662 7.59 12.193L7.784 12.352C8.097 12.609 8.49 12.75 8.896 12.75L9.074 12.75C9.483 12.75 9.88 12.607 10.194 12.345L10.368 12.2C11.01 11.665 11.941 11.659 12.589 12.185L12.805 12.359C13.117 12.612 13.506 12.75 13.907 12.75L14.044 12.75C14.45 12.75 14.844 12.608 15.158 12.35L15.343 12.197C15.989 11.663 16.924 11.663 17.571 12.197L17.755 12.35C18.068 12.608 18.462 12.75 18.869 12.75L19.095 12.75C19.504 12.75 19.901 12.606 20.216 12.344L20.38 12.208C21.028 11.666 21.972 11.666 22.62 12.207L23.022 12.542C23.183 12.676 23.387 12.75 23.598 12.75 24.097 12.75 24.5 12.347 24.5 11.85L24.5 10.25ZM24 14.217 24 23C24 24.243 22.993 25.25 21.75 25.25L6.25 25.25C5.007 25.25 4 24.243 4 23L4 14.236C2.875 14.112 2 13.158 2 12L2 9.951C2 9.272 2.12 8.6 2.354 7.964L3.518 4.802C4.01 3.563 5.207 2.75 6.54 2.75L21.461 2.75C22.793 2.75 23.99 3.563 24.488 4.819L25.646 7.964C25.88 8.6 26 9.272 26 9.951L26 11.85C26 13.039 25.135 14.026 24 14.217ZM16 23.75 16 17.75 12 17.75 12 23.75 16 23.75Z"></path></svg>
                </a>
            </li>
            <li class="icon_link">
                <a href="groups">
                    <svg viewBox="0 0 28 28" class="a8c37x1j ms05siws l3qrxjdp b7h9ocf4 py1f6qlh" fill="#606266" height="28" width="28"><path d="M25.5 14C25.5 7.649 20.351 2.5 14 2.5 7.649 2.5 2.5 7.649 2.5 14 2.5 20.351 7.649 25.5 14 25.5 20.351 25.5 25.5 20.351 25.5 14ZM27 14C27 21.18 21.18 27 14 27 6.82 27 1 21.18 1 14 1 6.82 6.82 1 14 1 21.18 1 27 6.82 27 14ZM7.479 14 7.631 14C7.933 14 8.102 14.338 7.934 14.591 7.334 15.491 6.983 16.568 6.983 17.724L6.983 18.221C6.983 18.342 6.99 18.461 7.004 18.578 7.03 18.802 6.862 19 6.637 19L6.123 19C5.228 19 4.5 18.25 4.5 17.327 4.5 15.492 5.727 14 7.479 14ZM20.521 14C22.274 14 23.5 15.492 23.5 17.327 23.5 18.25 22.772 19 21.878 19L21.364 19C21.139 19 20.97 18.802 20.997 18.578 21.01 18.461 21.017 18.342 21.017 18.221L21.017 17.724C21.017 16.568 20.667 15.491 20.067 14.591 19.899 14.338 20.067 14 20.369 14L20.521 14ZM8.25 13C7.147 13 6.25 11.991 6.25 10.75 6.25 9.384 7.035 8.5 8.25 8.5 9.465 8.5 10.25 9.384 10.25 10.75 10.25 11.991 9.353 13 8.25 13ZM19.75 13C18.647 13 17.75 11.991 17.75 10.75 17.75 9.384 18.535 8.5 19.75 8.5 20.965 8.5 21.75 9.384 21.75 10.75 21.75 11.991 20.853 13 19.75 13ZM15.172 13.5C17.558 13.5 19.5 15.395 19.5 17.724L19.5 18.221C19.5 19.202 18.683 20 17.677 20L10.323 20C9.317 20 8.5 19.202 8.5 18.221L8.5 17.724C8.5 15.395 10.441 13.5 12.828 13.5L15.172 13.5ZM16.75 9C16.75 10.655 15.517 12 14 12 12.484 12 11.25 10.655 11.25 9 11.25 7.15 12.304 6 14 6 15.697 6 16.75 7.15 16.75 9Z"></path></svg>
                </a>
            </li>
            <li class="icon_link">
                <a href="games">
                    <svg viewBox="0 0 28 28" class="a8c37x1j ms05siws l3qrxjdp b7h9ocf4 py1f6qlh" fill="#606266" height="28" width="28"><path d="M23.5 9.5H10.25a.75.75 0 00-.75.75v7c0 .414.336.75.75.75H17v5.5H4.5v-19h19v5zm0 14h-5v-6.25a.75.75 0 00-.75-.75H11V11h12.5v12.5zm1.5.25V4.25C25 3.561 24.439 3 23.75 3H4.25C3.561 3 3 3.561 3 4.25v19.5c0 .689.561 1.25 1.25 1.25h19.5c.689 0 1.25-.561 1.25-1.25z"></path></svg>
                </a>
            </li>
        </ul>
    </div>
    <!-- Mid Nav End -->

    <!-- Right Nav Start -->
    <div class="nav_right"> 
        <div class="user">
            <a href="/profile">        
                <div style="background-color: white;" class="image_container">
                    <img src="{{auth()->user()->profile->profile_pic}}" alt="" />
                </div>
                @auth
                    <h4>{{auth()->user()->fname}}</h4>
                @endauth
            </a>
        </div>
        <div class="nav_content">
            <svg width="20" height="20" viewBox="0 0 152 152" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="132" cy="20" r="20" transform="rotate(90 132 20)" fill="#1E1E1E"/>
                <circle cx="20" cy="132" r="20" transform="rotate(90 20 132)" fill="#1E1E1E"/>
                <circle cx="20" cy="76" r="20" transform="rotate(90 20 76)" fill="#1E1E1E"/>
                <circle cx="20" cy="20" r="20" transform="rotate(90 20 20)" fill="#1E1E1E"/>
                <circle cx="76" cy="132" r="20" transform="rotate(90 76 132)" fill="#1E1E1E"/>
                <circle cx="76" cy="76" r="20" transform="rotate(90 76 76)" fill="#1E1E1E"/>
                <circle cx="76" cy="20" r="20" transform="rotate(90 76 20)" fill="#1E1E1E"/>
                <circle cx="132" cy="132" r="20" transform="rotate(90 132 132)" fill="#1E1E1E"/>
                <circle cx="132" cy="76" r="20" transform="rotate(90 132 76)" fill="#1E1E1E"/>
            </svg>
        </div>
        <div class="nav_content">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M256.55 8C116.52 8 8 110.34 8 248.57c0 72.3 29.71 134.78 78.07 177.94 8.35 7.51 6.63 11.86 8.05 58.23A19.92 19.92 0 0 0 122 502.31c52.91-23.3 53.59-25.14 62.56-22.7C337.85 521.8 504 423.7 504 248.57 504 110.34 396.59 8 256.55 8zm149.24 185.13l-73 115.57a37.37 37.37 0 0 1-53.91 9.93l-58.08-43.47a15 15 0 0 0-18 0l-78.37 59.44c-10.46 7.93-24.16-4.6-17.11-15.67l73-115.57a37.36 37.36 0 0 1 53.91-9.93l58.06 43.46a15 15 0 0 0 18 0l78.41-59.38c10.44-7.98 24.14 4.54 17.09 15.62z"/></svg>
        </div>
        <div class="nav_content">
            <svg viewBox="0 0 28 28" alt="" class="a8c37x1j ms05siws l3qrxjdp b7h9ocf4 rs22bh7c" fill="currentColor" height="20" width="20"><path d="M7.847 23.488C9.207 23.488 11.443 23.363 14.467 22.806 13.944 24.228 12.581 25.247 10.98 25.247 9.649 25.247 8.483 24.542 7.825 23.488L7.847 23.488ZM24.923 15.73C25.17 17.002 24.278 18.127 22.27 19.076 21.17 19.595 18.724 20.583 14.684 21.369 11.568 21.974 9.285 22.113 7.848 22.113 7.421 22.113 7.068 22.101 6.79 22.085 4.574 21.958 3.324 21.248 3.077 19.976 2.702 18.049 3.295 17.305 4.278 16.073L4.537 15.748C5.2 14.907 5.459 14.081 5.035 11.902 4.086 7.022 6.284 3.687 11.064 2.753 15.846 1.83 19.134 4.096 20.083 8.977 20.506 11.156 21.056 11.824 21.986 12.355L21.986 12.356 22.348 12.561C23.72 13.335 24.548 13.802 24.923 15.73Z"></path></svg>                </div>
        <div class="logout_content">
            <button id="myBtn">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M310.6 246.6l-127.1 128C176.4 380.9 168.2 384 160 384s-16.38-3.125-22.63-9.375l-127.1-128C.2244 237.5-2.516 223.7 2.438 211.8S19.07 192 32 192h255.1c12.94 0 24.62 7.781 29.58 19.75S319.8 237.5 310.6 246.6z"/></svg>
            </button>
        </div>
    </div>
    <!-- Right Nav End -->

    <!-- Logout Card Start -->
    <div id="myLogoutCard" class="logout_card">
            <div class="profile">
                <div class="image_container">
                    <img style="height: inherit; object-fit:cover;" src="/{{auth()->user()->profile->profile_pic}}" alt="" />
                </div>
                <div class="info">
                    <h3>{{auth()->user()->fname}} {{auth()->user()->lname}}</h3>
                    <p><a href="/profile">See your profile</a></p>
                </div>
                <div class="close_logout_content">
                    <button id="myCloseBtn">
                        <i data-visualcompletion="css-img" class="hu5pjgll lzf7d6o1" style="background-image:url('https://static.xx.fbcdn.net/rsrc.php/v3/y_/r/2LbgkLQ1Cgn.png');background-position:-54px -118px;background-size:190px 164px;width:16px;height:16px;background-repeat:no-repeat;display:inline-block"></i>
                    </button>
                </div>
            </div>
            <div class="feedback">
                <div class="icon">
                    <i data-visualcompletion="css-img" class="hu5pjgll lzf7d6o1" style="background-image: url(&quot;https://static.xx.fbcdn.net/rsrc.php/v3/yW/r/YBE_1_nu88W.png&quot;); background-position: 0px -26px; background-size: 26px 158px; width: 20px; height: 20px; background-repeat: no-repeat; display: inline-block;"></i>
                </div>
                <div class="info">
                    <h3>Give feedback</h3>
                    <p><a href="/profile">Help us improve facebook</a></p>
                </div>
            </div>
            <div class="content">
                <div class="content_control">
                    <div class="left">
                        <div class="icon">
                            <i data-visualcompletion="css-img" class="hu5pjgll lzf7d6o1" style="background-image: url(&quot;https://static.xx.fbcdn.net/rsrc.php/v3/yU/r/QGp3rDWrU1s.png&quot;); background-position: -44px -236px; background-size: 74px 330px; width: 20px; height: 20px; background-repeat: no-repeat; display: inline-block;"></i>
                        </div>
                        <div class="info">
                            <h3>Settings & privacy</h3>
                        </div>
                    </div>
                    <div class="right">
                        <div class="icon">
                            <i data-visualcompletion="css-img" class="hu5pjgll m6k467ps" style="background-image: url(&quot;https://static.xx.fbcdn.net/rsrc.php/v3/y_/r/2LbgkLQ1Cgn.png&quot;); background-position: -112px -14px; background-size: 190px 164px; width: 24px; height: 24px; background-repeat: no-repeat; display: inline-block;"></i>
                        </div>
                    </div>
                </div>
                <div class="content_control">
                    <div class="left">
                        <div class="icon">
                            <i data-visualcompletion="css-img" class="hu5pjgll lzf7d6o1" style="background-image: url(&quot;https://static.xx.fbcdn.net/rsrc.php/v3/yH/r/FZrIS8ICcBx.png&quot;); background-position: 0px -198px; background-size: 22px 304px; width: 20px; height: 20px; background-repeat: no-repeat; display: inline-block;"></i>
                        </div>
                        <div class="info">
                            <h3>Help & Support</h3>
                        </div>
                    </div>
                    <div class="right">
                        <div class="icon">
                            <i data-visualcompletion="css-img" class="hu5pjgll m6k467ps" style="background-image: url(&quot;https://static.xx.fbcdn.net/rsrc.php/v3/y_/r/2LbgkLQ1Cgn.png&quot;); background-position: -112px -14px; background-size: 190px 164px; width: 24px; height: 24px; background-repeat: no-repeat; display: inline-block;"></i>
                        </div>
                    </div>
                </div>
                <div class="content_control">
                    <div class="left">
                        <div class="icon">
                            <i data-visualcompletion="css-img" class="hu5pjgll lzf7d6o1" style="background-image: url(&quot;https://static.xx.fbcdn.net/rsrc.php/v3/yl/r/p6HNu1UdH_J.png&quot;); background-position: 0px -420px; background-size: 26px 676px; width: 20px; height: 20px; background-repeat: no-repeat; display: inline-block;"></i>
                        </div>
                        <div class="info">
                            <h3>Display & Accessibility</h3>
                        </div>
                    </div>
                    <div class="right">
                        <div class="icon">
                            <i data-visualcompletion="css-img" class="hu5pjgll m6k467ps" style="background-image: url(&quot;https://static.xx.fbcdn.net/rsrc.php/v3/y_/r/2LbgkLQ1Cgn.png&quot;); background-position: -112px -14px; background-size: 190px 164px; width: 24px; height: 24px; background-repeat: no-repeat; display: inline-block;"></i>
                        </div>
                    </div>
                </div>
                <div class="logout_control">
                    <div class="icon">
                        <i data-visualcompletion="css-img" class="hu5pjgll lzf7d6o1" style="background-image: url(&quot;https://static.xx.fbcdn.net/rsrc.php/v3/yW/r/YBE_1_nu88W.png&quot;); background-position: 0px -48px; background-size: 26px 158px; width: 20px; height: 20px; background-repeat: no-repeat; display: inline-block;"><i data-visualcompletion="css-img" class="hu5pjgll lzf7d6o1" style="background-image: url(&quot;https://static.xx.fbcdn.net/rsrc.php/v3/yl/r/p6HNu1UdH_J.png&quot;); background-position: 0px -420px; background-size: 26px 676px; width: 20px; height: 20px; background-repeat: no-repeat; display: inline-block;"></i></i>
                    </div>
                    <div class="info">
                        <a style="text-decoration: none;" href="/logout">
                            <h3>Log Out</h3>
                        </a>
                    </div>
                </div>
            </div>
            <div class="terms" style="color:grey; font-size:13">
                Privacy  · Terms  · Advertising  · Ad Choices   · Cookies  · More · Meta © 2022
            </div>
        </div>
    <!-- Logout Card End -->
</nav>
<!-- Navigation Bar End -->